...
class MyVisitorWithConfig(Visitor[MyConfig]):
    def visit_ClassDef(self, node):
        self.error_from_node(MyError, node, thing=f"{node.name} {self.config.config_option}")
...
def test_assert_error_ok(visitor, config, expected_thing):
    assert_error(visitor, CODE_WITH_ERROR, MyError, config=config, thing=expected_thing)
